...
function AnotationTool({
  ...
  const showMarkerArea = () => {
    ...
    if (markerAreaState) {
      // if we have initial state, restore it
      ...
      markerArea.addRenderEventListener((dataUrl, state) => {
        if (sampleImageRef.current) {
          ...
          if (markers.some((marker) => marker.name.includes(' '))) {
            Toaster.notify(
              "The label can't include spaces.",
              ToasterType.failure
            );
            return;
          }

          const toasterId = Toaster.notify(
            'Creating an XML file for your image.',
            ToasterType.info
          );
          const data = {
          ...
          fetch(`${apiUrl}${isRotationAllowed ? 'rbb' : 'bb'}`, {
            ...
            .then((res) => {
              ...
              Toaster.update(
                toasterId,
                'The XML has been generated successfully.',
                ToasterType.success
              );
            })
            .catch((e) => {
  ...
  const getWidth = () => {
    ...
    return sampleImageRef.current.naturalWidth < offsetWidth

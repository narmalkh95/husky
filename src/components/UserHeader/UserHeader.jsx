...
export function CustomMenu(props) {
  ...
  return (
    <div className="custum__menu__wrapper" style={{ width: '20%' }}>
      <div className="user__close__btn" onClick={handleClick}>
      ...
      <div className="menu__items__wrapper">
        <div
          ...
          onClick={() =>
            history.push(
              path === paths.Importdata ? paths.Importdata : paths.DashBoard
            )
          }
        >
          dashboard
        </div>
        <div onClick={() => history.push(paths.Profile)}>
          profile and password
        </div>
        <div onClick={() => history.push(paths.Payment)}>payment details</div>
        <div onClick={() => history.push(paths.ContactUs)}>contact us</div>
        <div className="upgrade" onClick={() => history.push(paths.Upgrade)}>
          upgrade
        </div>

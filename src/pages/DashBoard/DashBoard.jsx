...
function DashBoard() {
  ...
  const createPicFoder = (data, isAbleToRedirect) => {
    fetch(`${apiUrl}create-folder?folder_name=${data.folder_name}`, {
      ...
      .then((response) => {
        ...
        if (
          response.status === 'fail' &&
          response.message === 'There is folder with that name.'
        ) {
  ...
  const addImages = (el) => {
    Promise.all(
      imagesArray.map((element) => new Promise((res) => {
                let i = new Image();
                i.onload = () => {
                    res({
  ...
  const handleCreate = (e) => {
    ...
    if (isValidFolderName(folder_name)) {
      !foldersNames.includes(folder_name) &&
        setFoldersNames((foldersNames) => folder_name && [...foldersNames, folder_name])
  ...
  const handleEditCreate = (el) => {
    ...
    if (!isValidFolderName(editFolderName)) {
      ...
      setFoldersNames((prev) =>
        prev.map((i) => (i === el ? editFolderName : i))
      );
      setOpenEdit(false);
      editFileName(data);
    }
      ...
      <div style={{ width: '100%', display: 'flex', flexDirection: 'row' }}>
        <div className={`profile_container ${showMenu}`}>
          ...
          <div className="dashboard__import__new__data__btn__wrapper">
            <div
              className="dashboard__import__new__data__btn"
              onClick={(el) => {
                setFolderName('');
        ...
        <Dialog
          ...
          onClose={() => {
            setNewImagesArray([]);
            setToggleAddImagePopup(false);
          }}
        >
          <div className="import__dialog__content__wrapper">
            <DialogTitle className="add__image__dialog__title">
              <span style={{ marginTop: '230px' }}>
                {' '}
                Upload more images to folder
              </span>
              <div style={{ color: '#8924BF' }}>{`${elementToAdd}`}</div>
            </DialogTitle>
            {!!newImagesArray.length && (
              <div className="selected__images__container">
                {newImagesArray.map((el) => (
                  <span className="add__image__item__name">
                    {el.file.name.length > maxLimitOfSelectedImages
                      ? el.file.name.substring(
                          0,
                          maxLimitOfSelectedImages - 3
                        ) + '...'
                      : el.file.name}
                  </span>
                ))}
            ...
            <DialogActions
              style={{ height: !!newImagesArray.length ? '85px' : '160px' }}
            >
              {!newImagesArray.length && (
                <ImageUploading
                  multiple
        ...
        >
          <div className="delete__dialog__content__wrapper">
            <DialogTitle className="edit__dialog__title">
              Edit folder name
            </DialogTitle>
            <>
              <DialogContent className="edit__dialog__content">
                ...
                />
                {folderNameErr && (
                  <div className="error__message" style={{ width: '100%' }}>
                    {folderNameErr}
                  </div>
                )}
              </DialogContent>
            ...
            <DialogActions>
              <button
                ...
                onClick={() => {
                  ...
                  let newArr = foldersNames.filter(
                    (el) => el !== elementToDelete
                  );
                  newArr.length
                    ? setFoldersNames(
                        foldersNames.filter((el) => el !== elementToDelete)
                      )

...
function Desktop() {
  ...
  const detectOnSingleImage = () => {
    setLoading(true);
    fetch(
      `${apiUrl}detect-on-single-image/${isRotationAllowed ? 'rbb' : 'bb'}`,
      {
        method: 'POST',
        headers: {
          'Content-type': 'application/json; charset=UTF-8',
            ...
            <div className="radio-buttons-container">
              <FormControl component="fieldset">
                <RadioGroup
                  ...
                  name="position"
                  defaultValue="top"
                  onChange={() => changeIsRotationAllowed()}
                >
                  <FormControlLabel
                    value="B-box"
                    control={<PurpleRadio checked={!isRotationAllowed} />}
                    label={
                      <Typography className="radio-buttons-label">
                        B-box
                      </Typography>
                    }
                  />

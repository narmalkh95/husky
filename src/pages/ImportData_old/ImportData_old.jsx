...
function ImportData_old(props) {
  ...
  const addImages = (el) => {
    Promise.all(
      newImagesArray.map((element) => new Promise((res) => {
                let i = new Image();
                i.onload = () => {
                    res({
                        image: element.data_url,
          ...
          <div className={`comp-import-data-${isDataExist}`}>
                ...
                {({ onImageUpload, errors }) => (
                  <span>
                    ...
                    {newImagesArray.length !== 0 && (
                      <div>{`YOU CHOOSED ${newImagesArray.length} PHOTOS`}</div>
                    )}
                    {errors && (
                      <div>
            ...
            <div className="import-data-container">
              ...
              <span className="small-header">
                To get started please import the data.
              </span>
              <button className="import-button" onClick={handleOpen}>
                  ...
                  {({ onImageUpload, errors }) => (
                    <span>
                      ...
                      {imagesArray.length !== 0 && (
                        <div>{`YOU CHOSSED ${imagesArray.length} PHOTOS`}</div>
                      )}
                      {errors && (
                        <div>
                          {errors.maxFileSize && (
                            <div> Maxfile size error </div>
                          )}
                          {errors.maxNumber && <div>MAX number error</div>}
                          {errors.resolution && <div>Resolution error</div>}
                        </div>
